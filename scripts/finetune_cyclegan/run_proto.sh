...
python $code_root/main.py --batch_size __BATCH_SIZE__ \
        ...
        --hidden_size __HIDDEN_SIZE__ --num_rnn_layers __NUM_RNN_LAYERS__ --use_lstm __USE_LSTM__ --use_attention __USE_ATTENTION__\
        --epochs __EPOCHS__ --lr __LEARNING_RATE__ --weight_decay __WEIGHT_DECAY__ \
        --dropout_p __DROPOUT_P__ --max_norm __MAX_NORM__ \
        --enable_scheduled_sampling __ENABLE_SCHEDULED_SAMPLING__ --scheduled_sampling_decay_type __SCHEDULED_SAMPLING_DECAY_TYPE__ \
        --scheduled_sampling_decay_factor __SCHEDULED_SAMPLING_DECAY_FACTOR__ --scheduled_sampling_min_eps __SCHEDULED_SAMPLING_MIN_EPS__ \
        --log_dir __LOG_DIR__ \
        ...
        --lamda_cos_ij __LAMDA_COS_IJ__ --skip_weight_decay __SKIP_WEIGHT_DECAY__ \
        --freeze_embeddings __FREEZE_EMBEDDINGS__ --clamp_lower __CLAMP_LOWER__ --clamp_upper __CLAMP_UPPER__ \
        --d_pretrain_num_epochs __D_PRETRAIN_NUM_EPOCHS__  --disc_recalibrate __DISC_RECALIBRATE__\
        --g_update_step_diff_recalib __G_UPDATE_STEP_DIFF_RECALIB__ \
